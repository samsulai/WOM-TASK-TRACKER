...
async function rolloverIncompleteTasks(weeksData, tasksData) {
  ...
  const weekStartById = new Map(weeksData.map((w) => [w.id, w.week_start]))
  const tasksToMove = tasksData.filter((t) => {
    const weekStart = weekStartById.get(t.week_id)
    return !t.done && weekStart && weekStart < currentWeekStart
  })
  ...
  let weeks = weeksData
  let currentWeek = weeksData.find((w) => w.week_start === currentWeekStart)

  if (!currentWeek) {
    const { data, error } = await supabase
      .from('weeks')
      .insert({ week_start: currentWeekStart, label: '' })
      .select()
      .single()
    if (error) {
      if (error.code === '23505') {
    ...
    if (!currentWeek) return { weeks: weeksData, tasks: tasksData, movedCount: 0 }
    weeks = [currentWeek, ...weeksData]
  }
  ...
  const { error: updateError } = await supabase
    ...
    .in('id', taskIds)
  if (updateError) {
    return { weeks, tasks: tasksData, movedCount: 0 }
  }

  const movedIds = new Set(taskIds)
  const tasks = tasksData.map((t) => (movedIds.has(t.id) ? { ...t, week_id: currentWeek.id } : t))
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function loadInitial() {
      ...
      if (cancelled) return
      if (weeksRes.error || tasksRes.error) {
        setLoadError((weeksRes.error || tasksRes.error).message)
        setLoading(false)
        return
      }

      const rolled = await rolloverIncompleteTasks(weeksRes.data, tasksRes.data)
      if (cancelled) return
  ...
  const flushWeekSave = useCallback(async (weekId) => {
    ...
    const { label, week_start } = week
    const { error } = await supabase.from('weeks').update({ label, week_start }).eq('id', weekId)
    if (error) {
      const message =
  ...
  const addWeek = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from('weeks')
      .insert({ week_start: weekStart, label: '' })
      .select()
      .single()
    if (error) {
      ...
      <header className="topbar">
        <div className="topbar-inner">
          <div className="topbar-brand">
            ...
            <div>
              <p className="eyebrow">Hours log</p>
              <span className="brand-name">Weekly Task Tracker</span>
            </div>
          </div>
